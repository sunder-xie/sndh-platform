...
public class SysContant {
    private static final String APP_PROD_CONFIG="env/app-dev.xml";
	...
	static{
		try{
			...
			InputStream appinputstream= SysContant.class.getClassLoader().getResourceAsStream("nhry.properties");
			Properties pros = new Properties();
            pros.load(appinputstream);
            String mode = pros.getProperty("app_mode");
            String appEnvConfig = APP_DEV_CONFIG;
            if(!StringUtils.isEmpty(mode) || "produce".equalsIgnoreCase(mode)){
            	appEnvConfig = APP_PROD_CONFIG;
            }
            pros.clear();
            pros = null;
           //获取根下面的 config,xml文件 进行加载读取
    		InputStream inputstream= SysContant.class.getClassLoader().getResourceAsStream(appEnvConfig);
