            ...
            if(details!=null){
                ...
                if(modelDeliver!=null){
                    for(TDispOrderItem deliverItem : modelDeliver){
                        ...
                        cell = row.createCell(1);
                        cell.setCellStyle(styleBoldwrap);
                        cell.setCellValue(deliverItem.getTotalQty()==null?"--":deliverItem.getAddressTxt().concat(":").concat(deliverItem.getTotalQty()==null?"" : deliverItem.getTotalQty().toString()).concat("--").concat(deliverItem.getMatnrTxt()));//小区名称
