...
public class BranchResource extends BaseResource {
	...
	@ApiOperation(value = "/search/{salesOrg}", response = NHSysParameter.class, notes = "根据销售组织查询网点客户信息列表")
	...
	@Produces(MediaType.APPLICATION_JSON)
	@ApiOperation(value = "/getBranchInfo/{branchNo}", response = NHSysParameter.class, notes = "根据网点编号查询网点客户信息列表")
	...
	@ApiOperation(value = "/list", response = NHSysParameter.class, notes = "根据销售组织、奶站性质、奶站等级查询网点客户(奶站)信息列表")
