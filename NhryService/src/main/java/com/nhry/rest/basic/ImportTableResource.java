                ...
                for (int s = 1; s <= rowNum1; s++) {
                    ...
                    if (cell1 != null && StringUtils.isNotEmpty(cell1.toString())) {
                        if (cell1.toString().equals(order.getOrderNo())) {
                            ...
                            throw new ServiceException("第" + (row1.getRowNum() + 1) + "行,订单号"+cell1.toString()+"不存在在订单行内，请检查！");
                        }
                    }
                }
                ...
                for (int s = 1; s <= rowNum1; s++) {
                    ...
                    if (cell1 != null && StringUtils.isNotEmpty(cell1.toString())) {
                        if (cell1.toString().equals(order.getOrderNo())) {
                            ...
                            throw new ServiceException("订单编号不一致，请检查：订单号" + order.getOrderNo() + "行项目订单号：" + cell1.toString());
                        }
                    }
                }
                OrderModel.setEntries(entries);
