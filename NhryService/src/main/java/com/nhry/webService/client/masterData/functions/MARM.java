/**
 * MARM.java
 *
 * This file was auto-generated from WSDL
 * by the Apache Axis2 version: 1.7.3  Built on : May 30, 2016 (04:09:26 BST)
 */
package com.nhry.webService.client.masterData.functions;


/**
 *  MARM bean class
 */
@SuppressWarnings({"unchecked",
    "unused"
})
public class MARM implements org.apache.axis2.databinding.ADBBean {
    /* This type was generated from the piece of schema that had
       name = MARM
       Namespace URI = urn:sap-com:document:sap:rfc:functions
       Namespace Prefix = ns1
     */

    /**
     * field for MANDT
     */
    protected MANDT_type1 localMANDT;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localMANDTTracker = false;

    /**
     * field for MATNR
     */
    protected MATNR_type5 localMATNR;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localMATNRTracker = false;

    /**
     * field for MEINH
     */
    protected MEINH_type1 localMEINH;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localMEINHTracker = false;

    /**
     * field for UMREZ
     */
    protected UMREZ_type1 localUMREZ;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localUMREZTracker = false;

    /**
     * field for UMREN
     */
    protected UMREN_type1 localUMREN;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localUMRENTracker = false;

    /**
     * field for EANNR
     */
    protected EANNR_type1 localEANNR;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localEANNRTracker = false;

    /**
     * field for EAN11
     */
    protected EAN11_type1 localEAN11;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localEAN11Tracker = false;

    /**
     * field for NUMTP
     */
    protected NUMTP_type1 localNUMTP;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localNUMTPTracker = false;

    /**
     * field for LAENG
     */
    protected LAENG_type1 localLAENG;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localLAENGTracker = false;

    /**
     * field for BREIT
     */
    protected BREIT_type1 localBREIT;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localBREITTracker = false;

    /**
     * field for HOEHE
     */
    protected HOEHE_type1 localHOEHE;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localHOEHETracker = false;

    /**
     * field for MEABM
     */
    protected MEABM_type1 localMEABM;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localMEABMTracker = false;

    /**
     * field for VOLUM
     */
    protected VOLUM_type1 localVOLUM;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localVOLUMTracker = false;

    /**
     * field for VOLEH
     */
    protected VOLEH_type1 localVOLEH;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localVOLEHTracker = false;

    /**
     * field for BRGEW
     */
    protected BRGEW_type1 localBRGEW;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localBRGEWTracker = false;

    /**
     * field for GEWEI
     */
    protected GEWEI_type1 localGEWEI;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localGEWEITracker = false;

    /**
     * field for MESUB
     */
    protected MESUB_type1 localMESUB;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localMESUBTracker = false;

    /**
     * field for ATINN
     */
    protected ATINN_type1 localATINN;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localATINNTracker = false;

    /**
     * field for MESRT
     */
    protected MESRT_type1 localMESRT;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localMESRTTracker = false;

    /**
     * field for XFHDW
     */
    protected XFHDW_type1 localXFHDW;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localXFHDWTracker = false;

    /**
     * field for XBEWW
     */
    protected XBEWW_type1 localXBEWW;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localXBEWWTracker = false;

    /**
     * field for KZWSO
     */
    protected KZWSO_type1 localKZWSO;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localKZWSOTracker = false;

    /**
     * field for MSEHI
     */
    protected MSEHI_type1 localMSEHI;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localMSEHITracker = false;

    /**
     * field for BFLME_MARM
     */
    protected BFLME_MARM_type1 localBFLME_MARM;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localBFLME_MARMTracker = false;

    /**
     * field for GTIN_VARIANT
     */
    protected GTIN_VARIANT_type1 localGTIN_VARIANT;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localGTIN_VARIANTTracker = false;

    /**
     * field for NEST_FTR
     */
    protected NEST_FTR_type1 localNEST_FTR;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localNEST_FTRTracker = false;

    /**
     * field for MAX_STACK
     */
    protected org.apache.axis2.databinding.types.UnsignedByte localMAX_STACK;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localMAX_STACKTracker = false;

    /**
     * field for CAPAUSE
     */
    protected CAPAUSE_type1 localCAPAUSE;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localCAPAUSETracker = false;

    /**
     * field for TY2TQ
     */
    protected TY2TQ_type1 localTY2TQ;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localTY2TQTracker = false;

    public boolean isMANDTSpecified() {
        return localMANDTTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.MANDT_type1
     */
    public MANDT_type1 getMANDT() {
        return localMANDT;
    }

    /**
     * Auto generated setter method
     * @param param MANDT
     */
    public void setMANDT(MANDT_type1 param) {
        localMANDTTracker = param != null;

        this.localMANDT = param;
    }

    public boolean isMATNRSpecified() {
        return localMATNRTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.MATNR_type5
     */
    public MATNR_type5 getMATNR() {
        return localMATNR;
    }

    /**
     * Auto generated setter method
     * @param param MATNR
     */
    public void setMATNR(MATNR_type5 param) {
        localMATNRTracker = param != null;

        this.localMATNR = param;
    }

    public boolean isMEINHSpecified() {
        return localMEINHTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.MEINH_type1
     */
    public MEINH_type1 getMEINH() {
        return localMEINH;
    }

    /**
     * Auto generated setter method
     * @param param MEINH
     */
    public void setMEINH(MEINH_type1 param) {
        localMEINHTracker = param != null;

        this.localMEINH = param;
    }

    public boolean isUMREZSpecified() {
        return localUMREZTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.UMREZ_type1
     */
    public UMREZ_type1 getUMREZ() {
        return localUMREZ;
    }

    /**
     * Auto generated setter method
     * @param param UMREZ
     */
    public void setUMREZ(UMREZ_type1 param) {
        localUMREZTracker = param != null;

        this.localUMREZ = param;
    }

    public boolean isUMRENSpecified() {
        return localUMRENTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.UMREN_type1
     */
    public UMREN_type1 getUMREN() {
        return localUMREN;
    }

    /**
     * Auto generated setter method
     * @param param UMREN
     */
    public void setUMREN(UMREN_type1 param) {
        localUMRENTracker = param != null;

        this.localUMREN = param;
    }

    public boolean isEANNRSpecified() {
        return localEANNRTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.EANNR_type1
     */
    public EANNR_type1 getEANNR() {
        return localEANNR;
    }

    /**
     * Auto generated setter method
     * @param param EANNR
     */
    public void setEANNR(EANNR_type1 param) {
        localEANNRTracker = param != null;

        this.localEANNR = param;
    }

    public boolean isEAN11Specified() {
        return localEAN11Tracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.EAN11_type1
     */
    public EAN11_type1 getEAN11() {
        return localEAN11;
    }

    /**
     * Auto generated setter method
     * @param param EAN11
     */
    public void setEAN11(EAN11_type1 param) {
        localEAN11Tracker = param != null;

        this.localEAN11 = param;
    }

    public boolean isNUMTPSpecified() {
        return localNUMTPTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.NUMTP_type1
     */
    public NUMTP_type1 getNUMTP() {
        return localNUMTP;
    }

    /**
     * Auto generated setter method
     * @param param NUMTP
     */
    public void setNUMTP(NUMTP_type1 param) {
        localNUMTPTracker = param != null;

        this.localNUMTP = param;
    }

    public boolean isLAENGSpecified() {
        return localLAENGTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.LAENG_type1
     */
    public LAENG_type1 getLAENG() {
        return localLAENG;
    }

    /**
     * Auto generated setter method
     * @param param LAENG
     */
    public void setLAENG(LAENG_type1 param) {
        localLAENGTracker = param != null;

        this.localLAENG = param;
    }

    public boolean isBREITSpecified() {
        return localBREITTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.BREIT_type1
     */
    public BREIT_type1 getBREIT() {
        return localBREIT;
    }

    /**
     * Auto generated setter method
     * @param param BREIT
     */
    public void setBREIT(BREIT_type1 param) {
        localBREITTracker = param != null;

        this.localBREIT = param;
    }

    public boolean isHOEHESpecified() {
        return localHOEHETracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.HOEHE_type1
     */
    public HOEHE_type1 getHOEHE() {
        return localHOEHE;
    }

    /**
     * Auto generated setter method
     * @param param HOEHE
     */
    public void setHOEHE(HOEHE_type1 param) {
        localHOEHETracker = param != null;

        this.localHOEHE = param;
    }

    public boolean isMEABMSpecified() {
        return localMEABMTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.MEABM_type1
     */
    public MEABM_type1 getMEABM() {
        return localMEABM;
    }

    /**
     * Auto generated setter method
     * @param param MEABM
     */
    public void setMEABM(MEABM_type1 param) {
        localMEABMTracker = param != null;

        this.localMEABM = param;
    }

    public boolean isVOLUMSpecified() {
        return localVOLUMTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.VOLUM_type1
     */
    public VOLUM_type1 getVOLUM() {
        return localVOLUM;
    }

    /**
     * Auto generated setter method
     * @param param VOLUM
     */
    public void setVOLUM(VOLUM_type1 param) {
        localVOLUMTracker = param != null;

        this.localVOLUM = param;
    }

    public boolean isVOLEHSpecified() {
        return localVOLEHTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.VOLEH_type1
     */
    public VOLEH_type1 getVOLEH() {
        return localVOLEH;
    }

    /**
     * Auto generated setter method
     * @param param VOLEH
     */
    public void setVOLEH(VOLEH_type1 param) {
        localVOLEHTracker = param != null;

        this.localVOLEH = param;
    }

    public boolean isBRGEWSpecified() {
        return localBRGEWTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.BRGEW_type1
     */
    public BRGEW_type1 getBRGEW() {
        return localBRGEW;
    }

    /**
     * Auto generated setter method
     * @param param BRGEW
     */
    public void setBRGEW(BRGEW_type1 param) {
        localBRGEWTracker = param != null;

        this.localBRGEW = param;
    }

    public boolean isGEWEISpecified() {
        return localGEWEITracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.GEWEI_type1
     */
    public GEWEI_type1 getGEWEI() {
        return localGEWEI;
    }

    /**
     * Auto generated setter method
     * @param param GEWEI
     */
    public void setGEWEI(GEWEI_type1 param) {
        localGEWEITracker = param != null;

        this.localGEWEI = param;
    }

    public boolean isMESUBSpecified() {
        return localMESUBTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.MESUB_type1
     */
    public MESUB_type1 getMESUB() {
        return localMESUB;
    }

    /**
     * Auto generated setter method
     * @param param MESUB
     */
    public void setMESUB(MESUB_type1 param) {
        localMESUBTracker = param != null;

        this.localMESUB = param;
    }

    public boolean isATINNSpecified() {
        return localATINNTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.ATINN_type1
     */
    public ATINN_type1 getATINN() {
        return localATINN;
    }

    /**
     * Auto generated setter method
     * @param param ATINN
     */
    public void setATINN(ATINN_type1 param) {
        localATINNTracker = param != null;

        this.localATINN = param;
    }

    public boolean isMESRTSpecified() {
        return localMESRTTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.MESRT_type1
     */
    public MESRT_type1 getMESRT() {
        return localMESRT;
    }

    /**
     * Auto generated setter method
     * @param param MESRT
     */
    public void setMESRT(MESRT_type1 param) {
        localMESRTTracker = param != null;

        this.localMESRT = param;
    }

    public boolean isXFHDWSpecified() {
        return localXFHDWTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.XFHDW_type1
     */
    public XFHDW_type1 getXFHDW() {
        return localXFHDW;
    }

    /**
     * Auto generated setter method
     * @param param XFHDW
     */
    public void setXFHDW(XFHDW_type1 param) {
        localXFHDWTracker = param != null;

        this.localXFHDW = param;
    }

    public boolean isXBEWWSpecified() {
        return localXBEWWTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.XBEWW_type1
     */
    public XBEWW_type1 getXBEWW() {
        return localXBEWW;
    }

    /**
     * Auto generated setter method
     * @param param XBEWW
     */
    public void setXBEWW(XBEWW_type1 param) {
        localXBEWWTracker = param != null;

        this.localXBEWW = param;
    }

    public boolean isKZWSOSpecified() {
        return localKZWSOTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.KZWSO_type1
     */
    public KZWSO_type1 getKZWSO() {
        return localKZWSO;
    }

    /**
     * Auto generated setter method
     * @param param KZWSO
     */
    public void setKZWSO(KZWSO_type1 param) {
        localKZWSOTracker = param != null;

        this.localKZWSO = param;
    }

    public boolean isMSEHISpecified() {
        return localMSEHITracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.MSEHI_type1
     */
    public MSEHI_type1 getMSEHI() {
        return localMSEHI;
    }

    /**
     * Auto generated setter method
     * @param param MSEHI
     */
    public void setMSEHI(MSEHI_type1 param) {
        localMSEHITracker = param != null;

        this.localMSEHI = param;
    }

    public boolean isBFLME_MARMSpecified() {
        return localBFLME_MARMTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.BFLME_MARM_type1
     */
    public BFLME_MARM_type1 getBFLME_MARM() {
        return localBFLME_MARM;
    }

    /**
     * Auto generated setter method
     * @param param BFLME_MARM
     */
    public void setBFLME_MARM(
        BFLME_MARM_type1 param) {
        localBFLME_MARMTracker = param != null;

        this.localBFLME_MARM = param;
    }

    public boolean isGTIN_VARIANTSpecified() {
        return localGTIN_VARIANTTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.GTIN_VARIANT_type1
     */
    public GTIN_VARIANT_type1 getGTIN_VARIANT() {
        return localGTIN_VARIANT;
    }

    /**
     * Auto generated setter method
     * @param param GTIN_VARIANT
     */
    public void setGTIN_VARIANT(
        GTIN_VARIANT_type1 param) {
        localGTIN_VARIANTTracker = param != null;

        this.localGTIN_VARIANT = param;
    }

    public boolean isNEST_FTRSpecified() {
        return localNEST_FTRTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.NEST_FTR_type1
     */
    public NEST_FTR_type1 getNEST_FTR() {
        return localNEST_FTR;
    }

    /**
     * Auto generated setter method
     * @param param NEST_FTR
     */
    public void setNEST_FTR(
        NEST_FTR_type1 param) {
        localNEST_FTRTracker = param != null;

        this.localNEST_FTR = param;
    }

    public boolean isMAX_STACKSpecified() {
        return localMAX_STACKTracker;
    }

    /**
     * Auto generated getter method
     * @return org.apache.axis2.databinding.types.UnsignedByte
     */
    public org.apache.axis2.databinding.types.UnsignedByte getMAX_STACK() {
        return localMAX_STACK;
    }

    /**
     * Auto generated setter method
     * @param param MAX_STACK
     */
    public void setMAX_STACK(
        org.apache.axis2.databinding.types.UnsignedByte param) {
        localMAX_STACKTracker = param != null;

        this.localMAX_STACK = param;
    }

    public boolean isCAPAUSESpecified() {
        return localCAPAUSETracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.CAPAUSE_type1
     */
    public CAPAUSE_type1 getCAPAUSE() {
        return localCAPAUSE;
    }

    /**
     * Auto generated setter method
     * @param param CAPAUSE
     */
    public void setCAPAUSE(
        CAPAUSE_type1 param) {
        localCAPAUSETracker = param != null;

        this.localCAPAUSE = param;
    }

    public boolean isTY2TQSpecified() {
        return localTY2TQTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.TY2TQ_type1
     */
    public TY2TQ_type1 getTY2TQ() {
        return localTY2TQ;
    }

    /**
     * Auto generated setter method
     * @param param TY2TQ
     */
    public void setTY2TQ(TY2TQ_type1 param) {
        localTY2TQTracker = param != null;

        this.localTY2TQ = param;
    }

    /**
     *
     * @param parentQName
     * @param factory
     * @return org.apache.axiom.om.OMElement
     */
    public org.apache.axiom.om.OMElement getOMElement(
        final javax.xml.namespace.QName parentQName,
        final org.apache.axiom.om.OMFactory factory)
        throws org.apache.axis2.databinding.ADBException {
        return factory.createOMElement(new org.apache.axis2.databinding.ADBDataSource(
                this, parentQName));
    }

    public void serialize(final javax.xml.namespace.QName parentQName,
        javax.xml.stream.XMLStreamWriter xmlWriter)
        throws javax.xml.stream.XMLStreamException,
            org.apache.axis2.databinding.ADBException {
        serialize(parentQName, xmlWriter, false);
    }

    public void serialize(final javax.xml.namespace.QName parentQName,
        javax.xml.stream.XMLStreamWriter xmlWriter, boolean serializeType)
        throws javax.xml.stream.XMLStreamException,
            org.apache.axis2.databinding.ADBException {
        String prefix = null;
        String namespace = null;

        prefix = parentQName.getPrefix();
        namespace = parentQName.getNamespaceURI();
        writeStartElement(prefix, namespace, parentQName.getLocalPart(),
            xmlWriter);

        if (serializeType) {
            String namespacePrefix = registerPrefix(xmlWriter,
                    "urn:sap-com:document:sap:rfc:functions");

            if ((namespacePrefix != null) &&
                    (namespacePrefix.trim().length() > 0)) {
                writeAttribute("xsi",
                    "http://www.w3.org/2001/XMLSchema-instance", "type",
                    namespacePrefix + ":MARM", xmlWriter);
            } else {
                writeAttribute("xsi",
                    "http://www.w3.org/2001/XMLSchema-instance", "type",
                    "MARM", xmlWriter);
            }
        }

        if (localMANDTTracker) {
            if (localMANDT == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "MANDT cannot be null!!");
            }

            localMANDT.serialize(new javax.xml.namespace.QName("", "MANDT"),
                xmlWriter);
        }

        if (localMATNRTracker) {
            if (localMATNR == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "MATNR cannot be null!!");
            }

            localMATNR.serialize(new javax.xml.namespace.QName("", "MATNR"),
                xmlWriter);
        }

        if (localMEINHTracker) {
            if (localMEINH == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "MEINH cannot be null!!");
            }

            localMEINH.serialize(new javax.xml.namespace.QName("", "MEINH"),
                xmlWriter);
        }

        if (localUMREZTracker) {
            if (localUMREZ == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "UMREZ cannot be null!!");
            }

            localUMREZ.serialize(new javax.xml.namespace.QName("", "UMREZ"),
                xmlWriter);
        }

        if (localUMRENTracker) {
            if (localUMREN == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "UMREN cannot be null!!");
            }

            localUMREN.serialize(new javax.xml.namespace.QName("", "UMREN"),
                xmlWriter);
        }

        if (localEANNRTracker) {
            if (localEANNR == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "EANNR cannot be null!!");
            }

            localEANNR.serialize(new javax.xml.namespace.QName("", "EANNR"),
                xmlWriter);
        }

        if (localEAN11Tracker) {
            if (localEAN11 == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "EAN11 cannot be null!!");
            }

            localEAN11.serialize(new javax.xml.namespace.QName("", "EAN11"),
                xmlWriter);
        }

        if (localNUMTPTracker) {
            if (localNUMTP == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "NUMTP cannot be null!!");
            }

            localNUMTP.serialize(new javax.xml.namespace.QName("", "NUMTP"),
                xmlWriter);
        }

        if (localLAENGTracker) {
            if (localLAENG == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "LAENG cannot be null!!");
            }

            localLAENG.serialize(new javax.xml.namespace.QName("", "LAENG"),
                xmlWriter);
        }

        if (localBREITTracker) {
            if (localBREIT == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "BREIT cannot be null!!");
            }

            localBREIT.serialize(new javax.xml.namespace.QName("", "BREIT"),
                xmlWriter);
        }

        if (localHOEHETracker) {
            if (localHOEHE == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "HOEHE cannot be null!!");
            }

            localHOEHE.serialize(new javax.xml.namespace.QName("", "HOEHE"),
                xmlWriter);
        }

        if (localMEABMTracker) {
            if (localMEABM == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "MEABM cannot be null!!");
            }

            localMEABM.serialize(new javax.xml.namespace.QName("", "MEABM"),
                xmlWriter);
        }

        if (localVOLUMTracker) {
            if (localVOLUM == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "VOLUM cannot be null!!");
            }

            localVOLUM.serialize(new javax.xml.namespace.QName("", "VOLUM"),
                xmlWriter);
        }

        if (localVOLEHTracker) {
            if (localVOLEH == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "VOLEH cannot be null!!");
            }

            localVOLEH.serialize(new javax.xml.namespace.QName("", "VOLEH"),
                xmlWriter);
        }

        if (localBRGEWTracker) {
            if (localBRGEW == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "BRGEW cannot be null!!");
            }

            localBRGEW.serialize(new javax.xml.namespace.QName("", "BRGEW"),
                xmlWriter);
        }

        if (localGEWEITracker) {
            if (localGEWEI == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "GEWEI cannot be null!!");
            }

            localGEWEI.serialize(new javax.xml.namespace.QName("", "GEWEI"),
                xmlWriter);
        }

        if (localMESUBTracker) {
            if (localMESUB == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "MESUB cannot be null!!");
            }

            localMESUB.serialize(new javax.xml.namespace.QName("", "MESUB"),
                xmlWriter);
        }

        if (localATINNTracker) {
            if (localATINN == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "ATINN cannot be null!!");
            }

            localATINN.serialize(new javax.xml.namespace.QName("", "ATINN"),
                xmlWriter);
        }

        if (localMESRTTracker) {
            if (localMESRT == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "MESRT cannot be null!!");
            }

            localMESRT.serialize(new javax.xml.namespace.QName("", "MESRT"),
                xmlWriter);
        }

        if (localXFHDWTracker) {
            if (localXFHDW == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "XFHDW cannot be null!!");
            }

            localXFHDW.serialize(new javax.xml.namespace.QName("", "XFHDW"),
                xmlWriter);
        }

        if (localXBEWWTracker) {
            if (localXBEWW == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "XBEWW cannot be null!!");
            }

            localXBEWW.serialize(new javax.xml.namespace.QName("", "XBEWW"),
                xmlWriter);
        }

        if (localKZWSOTracker) {
            if (localKZWSO == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "KZWSO cannot be null!!");
            }

            localKZWSO.serialize(new javax.xml.namespace.QName("", "KZWSO"),
                xmlWriter);
        }

        if (localMSEHITracker) {
            if (localMSEHI == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "MSEHI cannot be null!!");
            }

            localMSEHI.serialize(new javax.xml.namespace.QName("", "MSEHI"),
                xmlWriter);
        }

        if (localBFLME_MARMTracker) {
            if (localBFLME_MARM == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "BFLME_MARM cannot be null!!");
            }

            localBFLME_MARM.serialize(new javax.xml.namespace.QName("",
                    "BFLME_MARM"), xmlWriter);
        }

        if (localGTIN_VARIANTTracker) {
            if (localGTIN_VARIANT == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "GTIN_VARIANT cannot be null!!");
            }

            localGTIN_VARIANT.serialize(new javax.xml.namespace.QName("",
                    "GTIN_VARIANT"), xmlWriter);
        }

        if (localNEST_FTRTracker) {
            if (localNEST_FTR == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "NEST_FTR cannot be null!!");
            }

            localNEST_FTR.serialize(new javax.xml.namespace.QName("", "NEST_FTR"),
                xmlWriter);
        }

        if (localMAX_STACKTracker) {
            namespace = "";
            writeStartElement(null, namespace, "MAX_STACK", xmlWriter);

            if (localMAX_STACK == null) {
                // write the nil attribute
                throw new org.apache.axis2.databinding.ADBException(
                    "MAX_STACK cannot be null!!");
            } else {
                xmlWriter.writeCharacters(org.apache.axis2.databinding.utils.ConverterUtil.convertToString(
                        localMAX_STACK));
            }

            xmlWriter.writeEndElement();
        }

        if (localCAPAUSETracker) {
            if (localCAPAUSE == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "CAPAUSE cannot be null!!");
            }

            localCAPAUSE.serialize(new javax.xml.namespace.QName("", "CAPAUSE"),
                xmlWriter);
        }

        if (localTY2TQTracker) {
            if (localTY2TQ == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "TY2TQ cannot be null!!");
            }

            localTY2TQ.serialize(new javax.xml.namespace.QName("", "TY2TQ"),
                xmlWriter);
        }

        xmlWriter.writeEndElement();
    }

    private static String generatePrefix(String namespace) {
        if (namespace.equals("urn:sap-com:document:sap:rfc:functions")) {
            return "ns1";
        }

        return org.apache.axis2.databinding.utils.BeanUtil.getUniquePrefix();
    }

    /**
     * Utility method to write an element start tag.
     */
    private void writeStartElement(String prefix,
        String namespace, String localPart,
        javax.xml.stream.XMLStreamWriter xmlWriter)
        throws javax.xml.stream.XMLStreamException {
        String writerPrefix = xmlWriter.getPrefix(namespace);

        if (writerPrefix != null) {
            xmlWriter.writeStartElement(writerPrefix, localPart, namespace);
        } else {
            if (namespace.length() == 0) {
                prefix = "";
            } else if (prefix == null) {
                prefix = generatePrefix(namespace);
            }

            xmlWriter.writeStartElement(prefix, localPart, namespace);
            xmlWriter.writeNamespace(prefix, namespace);
            xmlWriter.setPrefix(prefix, namespace);
        }
    }

    /**
     * Util method to write an attribute with the ns prefix
     */
    private void writeAttribute(String prefix,
        String namespace, String attName,
        String attValue, javax.xml.stream.XMLStreamWriter xmlWriter)
        throws javax.xml.stream.XMLStreamException {
        String writerPrefix = xmlWriter.getPrefix(namespace);

        if (writerPrefix != null) {
            xmlWriter.writeAttribute(writerPrefix, namespace, attName, attValue);
        } else {
            xmlWriter.writeNamespace(prefix, namespace);
            xmlWriter.setPrefix(prefix, namespace);
            xmlWriter.writeAttribute(prefix, namespace, attName, attValue);
        }
    }

    /**
     * Util method to write an attribute without the ns prefix
     */
    private void writeAttribute(String namespace,
        String attName, String attValue,
        javax.xml.stream.XMLStreamWriter xmlWriter)
        throws javax.xml.stream.XMLStreamException {
        if (namespace.equals("")) {
            xmlWriter.writeAttribute(attName, attValue);
        } else {
            xmlWriter.writeAttribute(registerPrefix(xmlWriter, namespace),
                namespace, attName, attValue);
        }
    }

    /**
     * Util method to write an attribute without the ns prefix
     */
    private void writeQNameAttribute(String namespace,
        String attName, javax.xml.namespace.QName qname,
        javax.xml.stream.XMLStreamWriter xmlWriter)
        throws javax.xml.stream.XMLStreamException {
        String attributeNamespace = qname.getNamespaceURI();
        String attributePrefix = xmlWriter.getPrefix(attributeNamespace);

        if (attributePrefix == null) {
            attributePrefix = registerPrefix(xmlWriter, attributeNamespace);
        }

        String attributeValue;

        if (attributePrefix.trim().length() > 0) {
            attributeValue = attributePrefix + ":" + qname.getLocalPart();
        } else {
            attributeValue = qname.getLocalPart();
        }

        if (namespace.equals("")) {
            xmlWriter.writeAttribute(attName, attributeValue);
        } else {
            registerPrefix(xmlWriter, namespace);
            xmlWriter.writeAttribute(attributePrefix, namespace, attName,
                attributeValue);
        }
    }

    /**
     *  method to handle Qnames
     */
    private void writeQName(javax.xml.namespace.QName qname,
        javax.xml.stream.XMLStreamWriter xmlWriter)
        throws javax.xml.stream.XMLStreamException {
        String namespaceURI = qname.getNamespaceURI();

        if (namespaceURI != null) {
            String prefix = xmlWriter.getPrefix(namespaceURI);

            if (prefix == null) {
                prefix = generatePrefix(namespaceURI);
                xmlWriter.writeNamespace(prefix, namespaceURI);
                xmlWriter.setPrefix(prefix, namespaceURI);
            }

            if (prefix.trim().length() > 0) {
                xmlWriter.writeCharacters(prefix + ":" +
                    org.apache.axis2.databinding.utils.ConverterUtil.convertToString(
                        qname));
            } else {
                // i.e this is the default namespace
                xmlWriter.writeCharacters(org.apache.axis2.databinding.utils.ConverterUtil.convertToString(
                        qname));
            }
        } else {
            xmlWriter.writeCharacters(org.apache.axis2.databinding.utils.ConverterUtil.convertToString(
                    qname));
        }
    }

    private void writeQNames(javax.xml.namespace.QName[] qnames,
        javax.xml.stream.XMLStreamWriter xmlWriter)
        throws javax.xml.stream.XMLStreamException {
        if (qnames != null) {
            // we have to store this data until last moment since it is not possible to write any
            // namespace data after writing the charactor data
            StringBuffer stringToWrite = new StringBuffer();
            String namespaceURI = null;
            String prefix = null;

            for (int i = 0; i < qnames.length; i++) {
                if (i > 0) {
                    stringToWrite.append(" ");
                }

                namespaceURI = qnames[i].getNamespaceURI();

                if (namespaceURI != null) {
                    prefix = xmlWriter.getPrefix(namespaceURI);

                    if ((prefix == null) || (prefix.length() == 0)) {
                        prefix = generatePrefix(namespaceURI);
                        xmlWriter.writeNamespace(prefix, namespaceURI);
                        xmlWriter.setPrefix(prefix, namespaceURI);
                    }

                    if (prefix.trim().length() > 0) {
                        stringToWrite.append(prefix).append(":")
                                     .append(org.apache.axis2.databinding.utils.ConverterUtil.convertToString(
                                qnames[i]));
                    } else {
                        stringToWrite.append(org.apache.axis2.databinding.utils.ConverterUtil.convertToString(
                                qnames[i]));
                    }
                } else {
                    stringToWrite.append(org.apache.axis2.databinding.utils.ConverterUtil.convertToString(
                            qnames[i]));
                }
            }

            xmlWriter.writeCharacters(stringToWrite.toString());
        }
    }

    /**
     * Register a namespace prefix
     */
    private String registerPrefix(
        javax.xml.stream.XMLStreamWriter xmlWriter, String namespace)
        throws javax.xml.stream.XMLStreamException {
        String prefix = xmlWriter.getPrefix(namespace);

        if (prefix == null) {
            prefix = generatePrefix(namespace);

            javax.xml.namespace.NamespaceContext nsContext = xmlWriter.getNamespaceContext();

            while (true) {
                String uri = nsContext.getNamespaceURI(prefix);

                if ((uri == null) || (uri.length() == 0)) {
                    break;
                }

                prefix = org.apache.axis2.databinding.utils.BeanUtil.getUniquePrefix();
            }

            xmlWriter.writeNamespace(prefix, namespace);
            xmlWriter.setPrefix(prefix, namespace);
        }

        return prefix;
    }

    /**
     *  Factory class that keeps the parse method
     */
    public static class Factory {
        private static org.apache.commons.logging.Log log = org.apache.commons.logging.LogFactory.getLog(Factory.class);

        /**
         * static method to create the object
         * Precondition:  If this object is an element, the current or next start element starts this object and any intervening reader events are ignorable
         *                If this object is not an element, it is a complex type and the reader is at the event just after the outer start element
         * Postcondition: If this object is an element, the reader is positioned at its end element
         *                If this object is a complex type, the reader is positioned at the end element of its outer element
         */
        public static MARM parse(javax.xml.stream.XMLStreamReader reader)
            throws Exception {
            MARM object = new MARM();

            int event;
            javax.xml.namespace.QName currentQName = null;
            String nillableValue = null;
            String prefix = "";
            String namespaceuri = "";

            try {
                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                currentQName = reader.getName();

                if (reader.getAttributeValue(
                            "http://www.w3.org/2001/XMLSchema-instance", "type") != null) {
                    String fullTypeName = reader.getAttributeValue("http://www.w3.org/2001/XMLSchema-instance",
                            "type");

                    if (fullTypeName != null) {
                        String nsPrefix = null;

                        if (fullTypeName.indexOf(":") > -1) {
                            nsPrefix = fullTypeName.substring(0,
                                    fullTypeName.indexOf(":"));
                        }

                        nsPrefix = (nsPrefix == null) ? "" : nsPrefix;

                        String type = fullTypeName.substring(fullTypeName.indexOf(
                                    ":") + 1);

                        if (!"MARM".equals(type)) {
                            //find namespace for the prefix
                            String nsUri = reader.getNamespaceContext()
                                                           .getNamespaceURI(nsPrefix);

                            return (MARM) ExtensionMapper.getTypeObject(nsUri,
                                type, reader);
                        }
                    }
                }

                // Note all attributes that were handled. Used to differ normal attributes
                // from anyAttributes.
                java.util.Vector handledAttributes = new java.util.Vector();

                reader.next();

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "MANDT").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "MANDT").equals(
                            reader.getName())) {
                    object.setMANDT(MANDT_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "MATNR").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "MATNR").equals(
                            reader.getName())) {
                    object.setMATNR(MATNR_type5.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "MEINH").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "MEINH").equals(
                            reader.getName())) {
                    object.setMEINH(MEINH_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "UMREZ").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "UMREZ").equals(
                            reader.getName())) {
                    object.setUMREZ(UMREZ_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "UMREN").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "UMREN").equals(
                            reader.getName())) {
                    object.setUMREN(UMREN_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "EANNR").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "EANNR").equals(
                            reader.getName())) {
                    object.setEANNR(EANNR_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "EAN11").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "EAN11").equals(
                            reader.getName())) {
                    object.setEAN11(EAN11_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "NUMTP").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "NUMTP").equals(
                            reader.getName())) {
                    object.setNUMTP(NUMTP_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "LAENG").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "LAENG").equals(
                            reader.getName())) {
                    object.setLAENG(LAENG_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "BREIT").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "BREIT").equals(
                            reader.getName())) {
                    object.setBREIT(BREIT_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "HOEHE").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "HOEHE").equals(
                            reader.getName())) {
                    object.setHOEHE(HOEHE_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "MEABM").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "MEABM").equals(
                            reader.getName())) {
                    object.setMEABM(MEABM_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "VOLUM").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "VOLUM").equals(
                            reader.getName())) {
                    object.setVOLUM(VOLUM_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "VOLEH").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "VOLEH").equals(
                            reader.getName())) {
                    object.setVOLEH(VOLEH_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "BRGEW").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "BRGEW").equals(
                            reader.getName())) {
                    object.setBRGEW(BRGEW_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "GEWEI").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "GEWEI").equals(
                            reader.getName())) {
                    object.setGEWEI(GEWEI_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "MESUB").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "MESUB").equals(
                            reader.getName())) {
                    object.setMESUB(MESUB_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "ATINN").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "ATINN").equals(
                            reader.getName())) {
                    object.setATINN(ATINN_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "MESRT").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "MESRT").equals(
                            reader.getName())) {
                    object.setMESRT(MESRT_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "XFHDW").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "XFHDW").equals(
                            reader.getName())) {
                    object.setXFHDW(XFHDW_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "XBEWW").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "XBEWW").equals(
                            reader.getName())) {
                    object.setXBEWW(XBEWW_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "KZWSO").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "KZWSO").equals(
                            reader.getName())) {
                    object.setKZWSO(KZWSO_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "MSEHI").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "MSEHI").equals(
                            reader.getName())) {
                    object.setMSEHI(MSEHI_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "BFLME_MARM").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "BFLME_MARM").equals(
                            reader.getName())) {
                    object.setBFLME_MARM(BFLME_MARM_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "GTIN_VARIANT").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "GTIN_VARIANT").equals(
                            reader.getName())) {
                    object.setGTIN_VARIANT(GTIN_VARIANT_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "NEST_FTR").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "NEST_FTR").equals(
                            reader.getName())) {
                    object.setNEST_FTR(NEST_FTR_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "MAX_STACK").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "MAX_STACK").equals(
                            reader.getName())) {
                    nillableValue = reader.getAttributeValue("http://www.w3.org/2001/XMLSchema-instance",
                            "nil");

                    if ("true".equals(nillableValue) ||
                            "1".equals(nillableValue)) {
                        throw new org.apache.axis2.databinding.ADBException(
                            "The element: " + "MAX_STACK" + "  cannot be null");
                    }

                    String content = reader.getElementText();

                    object.setMAX_STACK(org.apache.axis2.databinding.utils.ConverterUtil.convertToUnsignedByte(
                            content));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "CAPAUSE").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "CAPAUSE").equals(
                            reader.getName())) {
                    object.setCAPAUSE(CAPAUSE_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "TY2TQ").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "TY2TQ").equals(
                            reader.getName())) {
                    object.setTY2TQ(TY2TQ_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if (reader.isStartElement()) {
                    // 2 - A start element we are not expecting indicates a trailing invalid property
                    throw new org.apache.axis2.databinding.ADBException(
                        "Unexpected subelement " + reader.getName());
                }
            } catch (javax.xml.stream.XMLStreamException e) {
                throw new Exception(e);
            }

            return object;
        }
    } //end of factory class
}
