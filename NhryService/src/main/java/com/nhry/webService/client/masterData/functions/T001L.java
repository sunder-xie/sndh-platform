/**
 * T001L.java
 *
 * This file was auto-generated from WSDL
 * by the Apache Axis2 version: 1.7.3  Built on : May 30, 2016 (04:09:26 BST)
 */
package com.nhry.webService.client.masterData.functions;


/**
 *  T001L bean class
 */
@SuppressWarnings({"unchecked",
    "unused"
})
public class T001L implements org.apache.axis2.databinding.ADBBean {
    /* This type was generated from the piece of schema that had
       name = T001L
       Namespace URI = urn:sap-com:document:sap:rfc:functions
       Namespace Prefix = ns1
     */

    /**
     * field for MANDT
     */
    protected MANDT_type13 localMANDT;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localMANDTTracker = false;

    /**
     * field for WERKS
     */
    protected WERKS_type7 localWERKS;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localWERKSTracker = false;

    /**
     * field for LGORT
     */
    protected LGORT_type3 localLGORT;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localLGORTTracker = false;

    /**
     * field for LGOBE
     */
    protected LGOBE_type1 localLGOBE;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localLGOBETracker = false;

    /**
     * field for SPART
     */
    protected SPART_type7 localSPART;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localSPARTTracker = false;

    /**
     * field for XLONG
     */
    protected XLONG_type1 localXLONG;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localXLONGTracker = false;

    /**
     * field for XBUFX
     */
    protected XBUFX_type1 localXBUFX;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localXBUFXTracker = false;

    /**
     * field for DISKZ
     */
    protected DISKZ_type1 localDISKZ;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localDISKZTracker = false;

    /**
     * field for XBLGO
     */
    protected XBLGO_type1 localXBLGO;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localXBLGOTracker = false;

    /**
     * field for XRESS
     */
    protected XRESS_type1 localXRESS;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localXRESSTracker = false;

    /**
     * field for XHUPF
     */
    protected XHUPF_type1 localXHUPF;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localXHUPFTracker = false;

    /**
     * field for PARLG
     */
    protected PARLG_type1 localPARLG;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localPARLGTracker = false;

    /**
     * field for VKORG
     */
    protected VKORG_type9 localVKORG;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localVKORGTracker = false;

    /**
     * field for VTWEG
     */
    protected VTWEG_type9 localVTWEG;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localVTWEGTracker = false;

    /**
     * field for VSTEL
     */
    protected VSTEL_type1 localVSTEL;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localVSTELTracker = false;

    /**
     * field for LIFNR
     */
    protected LIFNR_type1 localLIFNR;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localLIFNRTracker = false;

    /**
     * field for KUNNR
     */
    protected KUNNR_type7 localKUNNR;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localKUNNRTracker = false;

    /**
     * field for MESBS
     */
    protected MESBS_type1 localMESBS;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localMESBSTracker = false;

    /**
     * field for MESST
     */
    protected MESST_type1 localMESST;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localMESSTTracker = false;

    /**
     * field for OIH_LICNO
     */
    protected OIH_LICNO_type1 localOIH_LICNO;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localOIH_LICNOTracker = false;

    /**
     * field for OIG_ITRFL
     */
    protected OIG_ITRFL_type1 localOIG_ITRFL;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localOIG_ITRFLTracker = false;

    /**
     * field for OIB_TNKASSIGN
     */
    protected OIB_TNKASSIGN_type1 localOIB_TNKASSIGN;

    /*  This tracker boolean wil be used to detect whether the user called the set method
     *   for this attribute. It will be used to determine whether to include this field
     *   in the serialized XML
     */
    protected boolean localOIB_TNKASSIGNTracker = false;

    public boolean isMANDTSpecified() {
        return localMANDTTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.MANDT_type13
     */
    public MANDT_type13 getMANDT() {
        return localMANDT;
    }

    /**
     * Auto generated setter method
     * @param param MANDT
     */
    public void setMANDT(MANDT_type13 param) {
        localMANDTTracker = param != null;

        this.localMANDT = param;
    }

    public boolean isWERKSSpecified() {
        return localWERKSTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.WERKS_type7
     */
    public WERKS_type7 getWERKS() {
        return localWERKS;
    }

    /**
     * Auto generated setter method
     * @param param WERKS
     */
    public void setWERKS(WERKS_type7 param) {
        localWERKSTracker = param != null;

        this.localWERKS = param;
    }

    public boolean isLGORTSpecified() {
        return localLGORTTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.LGORT_type3
     */
    public LGORT_type3 getLGORT() {
        return localLGORT;
    }

    /**
     * Auto generated setter method
     * @param param LGORT
     */
    public void setLGORT(LGORT_type3 param) {
        localLGORTTracker = param != null;

        this.localLGORT = param;
    }

    public boolean isLGOBESpecified() {
        return localLGOBETracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.LGOBE_type1
     */
    public LGOBE_type1 getLGOBE() {
        return localLGOBE;
    }

    /**
     * Auto generated setter method
     * @param param LGOBE
     */
    public void setLGOBE(LGOBE_type1 param) {
        localLGOBETracker = param != null;

        this.localLGOBE = param;
    }

    public boolean isSPARTSpecified() {
        return localSPARTTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.SPART_type7
     */
    public SPART_type7 getSPART() {
        return localSPART;
    }

    /**
     * Auto generated setter method
     * @param param SPART
     */
    public void setSPART(SPART_type7 param) {
        localSPARTTracker = param != null;

        this.localSPART = param;
    }

    public boolean isXLONGSpecified() {
        return localXLONGTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.XLONG_type1
     */
    public XLONG_type1 getXLONG() {
        return localXLONG;
    }

    /**
     * Auto generated setter method
     * @param param XLONG
     */
    public void setXLONG(XLONG_type1 param) {
        localXLONGTracker = param != null;

        this.localXLONG = param;
    }

    public boolean isXBUFXSpecified() {
        return localXBUFXTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.XBUFX_type1
     */
    public XBUFX_type1 getXBUFX() {
        return localXBUFX;
    }

    /**
     * Auto generated setter method
     * @param param XBUFX
     */
    public void setXBUFX(XBUFX_type1 param) {
        localXBUFXTracker = param != null;

        this.localXBUFX = param;
    }

    public boolean isDISKZSpecified() {
        return localDISKZTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.DISKZ_type1
     */
    public DISKZ_type1 getDISKZ() {
        return localDISKZ;
    }

    /**
     * Auto generated setter method
     * @param param DISKZ
     */
    public void setDISKZ(DISKZ_type1 param) {
        localDISKZTracker = param != null;

        this.localDISKZ = param;
    }

    public boolean isXBLGOSpecified() {
        return localXBLGOTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.XBLGO_type1
     */
    public XBLGO_type1 getXBLGO() {
        return localXBLGO;
    }

    /**
     * Auto generated setter method
     * @param param XBLGO
     */
    public void setXBLGO(XBLGO_type1 param) {
        localXBLGOTracker = param != null;

        this.localXBLGO = param;
    }

    public boolean isXRESSSpecified() {
        return localXRESSTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.XRESS_type1
     */
    public XRESS_type1 getXRESS() {
        return localXRESS;
    }

    /**
     * Auto generated setter method
     * @param param XRESS
     */
    public void setXRESS(XRESS_type1 param) {
        localXRESSTracker = param != null;

        this.localXRESS = param;
    }

    public boolean isXHUPFSpecified() {
        return localXHUPFTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.XHUPF_type1
     */
    public XHUPF_type1 getXHUPF() {
        return localXHUPF;
    }

    /**
     * Auto generated setter method
     * @param param XHUPF
     */
    public void setXHUPF(XHUPF_type1 param) {
        localXHUPFTracker = param != null;

        this.localXHUPF = param;
    }

    public boolean isPARLGSpecified() {
        return localPARLGTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.PARLG_type1
     */
    public PARLG_type1 getPARLG() {
        return localPARLG;
    }

    /**
     * Auto generated setter method
     * @param param PARLG
     */
    public void setPARLG(PARLG_type1 param) {
        localPARLGTracker = param != null;

        this.localPARLG = param;
    }

    public boolean isVKORGSpecified() {
        return localVKORGTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.VKORG_type9
     */
    public VKORG_type9 getVKORG() {
        return localVKORG;
    }

    /**
     * Auto generated setter method
     * @param param VKORG
     */
    public void setVKORG(VKORG_type9 param) {
        localVKORGTracker = param != null;

        this.localVKORG = param;
    }

    public boolean isVTWEGSpecified() {
        return localVTWEGTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.VTWEG_type9
     */
    public VTWEG_type9 getVTWEG() {
        return localVTWEG;
    }

    /**
     * Auto generated setter method
     * @param param VTWEG
     */
    public void setVTWEG(VTWEG_type9 param) {
        localVTWEGTracker = param != null;

        this.localVTWEG = param;
    }

    public boolean isVSTELSpecified() {
        return localVSTELTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.VSTEL_type1
     */
    public VSTEL_type1 getVSTEL() {
        return localVSTEL;
    }

    /**
     * Auto generated setter method
     * @param param VSTEL
     */
    public void setVSTEL(VSTEL_type1 param) {
        localVSTELTracker = param != null;

        this.localVSTEL = param;
    }

    public boolean isLIFNRSpecified() {
        return localLIFNRTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.LIFNR_type1
     */
    public LIFNR_type1 getLIFNR() {
        return localLIFNR;
    }

    /**
     * Auto generated setter method
     * @param param LIFNR
     */
    public void setLIFNR(LIFNR_type1 param) {
        localLIFNRTracker = param != null;

        this.localLIFNR = param;
    }

    public boolean isKUNNRSpecified() {
        return localKUNNRTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.KUNNR_type7
     */
    public KUNNR_type7 getKUNNR() {
        return localKUNNR;
    }

    /**
     * Auto generated setter method
     * @param param KUNNR
     */
    public void setKUNNR(KUNNR_type7 param) {
        localKUNNRTracker = param != null;

        this.localKUNNR = param;
    }

    public boolean isMESBSSpecified() {
        return localMESBSTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.MESBS_type1
     */
    public MESBS_type1 getMESBS() {
        return localMESBS;
    }

    /**
     * Auto generated setter method
     * @param param MESBS
     */
    public void setMESBS(MESBS_type1 param) {
        localMESBSTracker = param != null;

        this.localMESBS = param;
    }

    public boolean isMESSTSpecified() {
        return localMESSTTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.MESST_type1
     */
    public MESST_type1 getMESST() {
        return localMESST;
    }

    /**
     * Auto generated setter method
     * @param param MESST
     */
    public void setMESST(MESST_type1 param) {
        localMESSTTracker = param != null;

        this.localMESST = param;
    }

    public boolean isOIH_LICNOSpecified() {
        return localOIH_LICNOTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.OIH_LICNO_type1
     */
    public OIH_LICNO_type1 getOIH_LICNO() {
        return localOIH_LICNO;
    }

    /**
     * Auto generated setter method
     * @param param OIH_LICNO
     */
    public void setOIH_LICNO(
        OIH_LICNO_type1 param) {
        localOIH_LICNOTracker = param != null;

        this.localOIH_LICNO = param;
    }

    public boolean isOIG_ITRFLSpecified() {
        return localOIG_ITRFLTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.OIG_ITRFL_type1
     */
    public OIG_ITRFL_type1 getOIG_ITRFL() {
        return localOIG_ITRFL;
    }

    /**
     * Auto generated setter method
     * @param param OIG_ITRFL
     */
    public void setOIG_ITRFL(
        OIG_ITRFL_type1 param) {
        localOIG_ITRFLTracker = param != null;

        this.localOIG_ITRFL = param;
    }

    public boolean isOIB_TNKASSIGNSpecified() {
        return localOIB_TNKASSIGNTracker;
    }

    /**
     * Auto generated getter method
     * @return com.nhry.webService.client.masterData.functions.OIB_TNKASSIGN_type1
     */
    public OIB_TNKASSIGN_type1 getOIB_TNKASSIGN() {
        return localOIB_TNKASSIGN;
    }

    /**
     * Auto generated setter method
     * @param param OIB_TNKASSIGN
     */
    public void setOIB_TNKASSIGN(
        OIB_TNKASSIGN_type1 param) {
        localOIB_TNKASSIGNTracker = param != null;

        this.localOIB_TNKASSIGN = param;
    }

    /**
     *
     * @param parentQName
     * @param factory
     * @return org.apache.axiom.om.OMElement
     */
    public org.apache.axiom.om.OMElement getOMElement(
        final javax.xml.namespace.QName parentQName,
        final org.apache.axiom.om.OMFactory factory)
        throws org.apache.axis2.databinding.ADBException {
        return factory.createOMElement(new org.apache.axis2.databinding.ADBDataSource(
                this, parentQName));
    }

    public void serialize(final javax.xml.namespace.QName parentQName,
        javax.xml.stream.XMLStreamWriter xmlWriter)
        throws javax.xml.stream.XMLStreamException,
            org.apache.axis2.databinding.ADBException {
        serialize(parentQName, xmlWriter, false);
    }

    public void serialize(final javax.xml.namespace.QName parentQName,
        javax.xml.stream.XMLStreamWriter xmlWriter, boolean serializeType)
        throws javax.xml.stream.XMLStreamException,
            org.apache.axis2.databinding.ADBException {
        String prefix = null;
        String namespace = null;

        prefix = parentQName.getPrefix();
        namespace = parentQName.getNamespaceURI();
        writeStartElement(prefix, namespace, parentQName.getLocalPart(),
            xmlWriter);

        if (serializeType) {
            String namespacePrefix = registerPrefix(xmlWriter,
                    "urn:sap-com:document:sap:rfc:functions");

            if ((namespacePrefix != null) &&
                    (namespacePrefix.trim().length() > 0)) {
                writeAttribute("xsi",
                    "http://www.w3.org/2001/XMLSchema-instance", "type",
                    namespacePrefix + ":T001L", xmlWriter);
            } else {
                writeAttribute("xsi",
                    "http://www.w3.org/2001/XMLSchema-instance", "type",
                    "T001L", xmlWriter);
            }
        }

        if (localMANDTTracker) {
            if (localMANDT == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "MANDT cannot be null!!");
            }

            localMANDT.serialize(new javax.xml.namespace.QName("", "MANDT"),
                xmlWriter);
        }

        if (localWERKSTracker) {
            if (localWERKS == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "WERKS cannot be null!!");
            }

            localWERKS.serialize(new javax.xml.namespace.QName("", "WERKS"),
                xmlWriter);
        }

        if (localLGORTTracker) {
            if (localLGORT == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "LGORT cannot be null!!");
            }

            localLGORT.serialize(new javax.xml.namespace.QName("", "LGORT"),
                xmlWriter);
        }

        if (localLGOBETracker) {
            if (localLGOBE == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "LGOBE cannot be null!!");
            }

            localLGOBE.serialize(new javax.xml.namespace.QName("", "LGOBE"),
                xmlWriter);
        }

        if (localSPARTTracker) {
            if (localSPART == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "SPART cannot be null!!");
            }

            localSPART.serialize(new javax.xml.namespace.QName("", "SPART"),
                xmlWriter);
        }

        if (localXLONGTracker) {
            if (localXLONG == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "XLONG cannot be null!!");
            }

            localXLONG.serialize(new javax.xml.namespace.QName("", "XLONG"),
                xmlWriter);
        }

        if (localXBUFXTracker) {
            if (localXBUFX == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "XBUFX cannot be null!!");
            }

            localXBUFX.serialize(new javax.xml.namespace.QName("", "XBUFX"),
                xmlWriter);
        }

        if (localDISKZTracker) {
            if (localDISKZ == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "DISKZ cannot be null!!");
            }

            localDISKZ.serialize(new javax.xml.namespace.QName("", "DISKZ"),
                xmlWriter);
        }

        if (localXBLGOTracker) {
            if (localXBLGO == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "XBLGO cannot be null!!");
            }

            localXBLGO.serialize(new javax.xml.namespace.QName("", "XBLGO"),
                xmlWriter);
        }

        if (localXRESSTracker) {
            if (localXRESS == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "XRESS cannot be null!!");
            }

            localXRESS.serialize(new javax.xml.namespace.QName("", "XRESS"),
                xmlWriter);
        }

        if (localXHUPFTracker) {
            if (localXHUPF == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "XHUPF cannot be null!!");
            }

            localXHUPF.serialize(new javax.xml.namespace.QName("", "XHUPF"),
                xmlWriter);
        }

        if (localPARLGTracker) {
            if (localPARLG == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "PARLG cannot be null!!");
            }

            localPARLG.serialize(new javax.xml.namespace.QName("", "PARLG"),
                xmlWriter);
        }

        if (localVKORGTracker) {
            if (localVKORG == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "VKORG cannot be null!!");
            }

            localVKORG.serialize(new javax.xml.namespace.QName("", "VKORG"),
                xmlWriter);
        }

        if (localVTWEGTracker) {
            if (localVTWEG == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "VTWEG cannot be null!!");
            }

            localVTWEG.serialize(new javax.xml.namespace.QName("", "VTWEG"),
                xmlWriter);
        }

        if (localVSTELTracker) {
            if (localVSTEL == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "VSTEL cannot be null!!");
            }

            localVSTEL.serialize(new javax.xml.namespace.QName("", "VSTEL"),
                xmlWriter);
        }

        if (localLIFNRTracker) {
            if (localLIFNR == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "LIFNR cannot be null!!");
            }

            localLIFNR.serialize(new javax.xml.namespace.QName("", "LIFNR"),
                xmlWriter);
        }

        if (localKUNNRTracker) {
            if (localKUNNR == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "KUNNR cannot be null!!");
            }

            localKUNNR.serialize(new javax.xml.namespace.QName("", "KUNNR"),
                xmlWriter);
        }

        if (localMESBSTracker) {
            if (localMESBS == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "MESBS cannot be null!!");
            }

            localMESBS.serialize(new javax.xml.namespace.QName("", "MESBS"),
                xmlWriter);
        }

        if (localMESSTTracker) {
            if (localMESST == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "MESST cannot be null!!");
            }

            localMESST.serialize(new javax.xml.namespace.QName("", "MESST"),
                xmlWriter);
        }

        if (localOIH_LICNOTracker) {
            if (localOIH_LICNO == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "OIH_LICNO cannot be null!!");
            }

            localOIH_LICNO.serialize(new javax.xml.namespace.QName("",
                    "OIH_LICNO"), xmlWriter);
        }

        if (localOIG_ITRFLTracker) {
            if (localOIG_ITRFL == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "OIG_ITRFL cannot be null!!");
            }

            localOIG_ITRFL.serialize(new javax.xml.namespace.QName("",
                    "OIG_ITRFL"), xmlWriter);
        }

        if (localOIB_TNKASSIGNTracker) {
            if (localOIB_TNKASSIGN == null) {
                throw new org.apache.axis2.databinding.ADBException(
                    "OIB_TNKASSIGN cannot be null!!");
            }

            localOIB_TNKASSIGN.serialize(new javax.xml.namespace.QName("",
                    "OIB_TNKASSIGN"), xmlWriter);
        }

        xmlWriter.writeEndElement();
    }

    private static String generatePrefix(String namespace) {
        if (namespace.equals("urn:sap-com:document:sap:rfc:functions")) {
            return "ns1";
        }

        return org.apache.axis2.databinding.utils.BeanUtil.getUniquePrefix();
    }

    /**
     * Utility method to write an element start tag.
     */
    private void writeStartElement(String prefix,
        String namespace, String localPart,
        javax.xml.stream.XMLStreamWriter xmlWriter)
        throws javax.xml.stream.XMLStreamException {
        String writerPrefix = xmlWriter.getPrefix(namespace);

        if (writerPrefix != null) {
            xmlWriter.writeStartElement(writerPrefix, localPart, namespace);
        } else {
            if (namespace.length() == 0) {
                prefix = "";
            } else if (prefix == null) {
                prefix = generatePrefix(namespace);
            }

            xmlWriter.writeStartElement(prefix, localPart, namespace);
            xmlWriter.writeNamespace(prefix, namespace);
            xmlWriter.setPrefix(prefix, namespace);
        }
    }

    /**
     * Util method to write an attribute with the ns prefix
     */
    private void writeAttribute(String prefix,
        String namespace, String attName,
        String attValue, javax.xml.stream.XMLStreamWriter xmlWriter)
        throws javax.xml.stream.XMLStreamException {
        String writerPrefix = xmlWriter.getPrefix(namespace);

        if (writerPrefix != null) {
            xmlWriter.writeAttribute(writerPrefix, namespace, attName, attValue);
        } else {
            xmlWriter.writeNamespace(prefix, namespace);
            xmlWriter.setPrefix(prefix, namespace);
            xmlWriter.writeAttribute(prefix, namespace, attName, attValue);
        }
    }

    /**
     * Util method to write an attribute without the ns prefix
     */
    private void writeAttribute(String namespace,
        String attName, String attValue,
        javax.xml.stream.XMLStreamWriter xmlWriter)
        throws javax.xml.stream.XMLStreamException {
        if (namespace.equals("")) {
            xmlWriter.writeAttribute(attName, attValue);
        } else {
            xmlWriter.writeAttribute(registerPrefix(xmlWriter, namespace),
                namespace, attName, attValue);
        }
    }

    /**
     * Util method to write an attribute without the ns prefix
     */
    private void writeQNameAttribute(String namespace,
        String attName, javax.xml.namespace.QName qname,
        javax.xml.stream.XMLStreamWriter xmlWriter)
        throws javax.xml.stream.XMLStreamException {
        String attributeNamespace = qname.getNamespaceURI();
        String attributePrefix = xmlWriter.getPrefix(attributeNamespace);

        if (attributePrefix == null) {
            attributePrefix = registerPrefix(xmlWriter, attributeNamespace);
        }

        String attributeValue;

        if (attributePrefix.trim().length() > 0) {
            attributeValue = attributePrefix + ":" + qname.getLocalPart();
        } else {
            attributeValue = qname.getLocalPart();
        }

        if (namespace.equals("")) {
            xmlWriter.writeAttribute(attName, attributeValue);
        } else {
            registerPrefix(xmlWriter, namespace);
            xmlWriter.writeAttribute(attributePrefix, namespace, attName,
                attributeValue);
        }
    }

    /**
     *  method to handle Qnames
     */
    private void writeQName(javax.xml.namespace.QName qname,
        javax.xml.stream.XMLStreamWriter xmlWriter)
        throws javax.xml.stream.XMLStreamException {
        String namespaceURI = qname.getNamespaceURI();

        if (namespaceURI != null) {
            String prefix = xmlWriter.getPrefix(namespaceURI);

            if (prefix == null) {
                prefix = generatePrefix(namespaceURI);
                xmlWriter.writeNamespace(prefix, namespaceURI);
                xmlWriter.setPrefix(prefix, namespaceURI);
            }

            if (prefix.trim().length() > 0) {
                xmlWriter.writeCharacters(prefix + ":" +
                    org.apache.axis2.databinding.utils.ConverterUtil.convertToString(
                        qname));
            } else {
                // i.e this is the default namespace
                xmlWriter.writeCharacters(org.apache.axis2.databinding.utils.ConverterUtil.convertToString(
                        qname));
            }
        } else {
            xmlWriter.writeCharacters(org.apache.axis2.databinding.utils.ConverterUtil.convertToString(
                    qname));
        }
    }

    private void writeQNames(javax.xml.namespace.QName[] qnames,
        javax.xml.stream.XMLStreamWriter xmlWriter)
        throws javax.xml.stream.XMLStreamException {
        if (qnames != null) {
            // we have to store this data until last moment since it is not possible to write any
            // namespace data after writing the charactor data
            StringBuffer stringToWrite = new StringBuffer();
            String namespaceURI = null;
            String prefix = null;

            for (int i = 0; i < qnames.length; i++) {
                if (i > 0) {
                    stringToWrite.append(" ");
                }

                namespaceURI = qnames[i].getNamespaceURI();

                if (namespaceURI != null) {
                    prefix = xmlWriter.getPrefix(namespaceURI);

                    if ((prefix == null) || (prefix.length() == 0)) {
                        prefix = generatePrefix(namespaceURI);
                        xmlWriter.writeNamespace(prefix, namespaceURI);
                        xmlWriter.setPrefix(prefix, namespaceURI);
                    }

                    if (prefix.trim().length() > 0) {
                        stringToWrite.append(prefix).append(":")
                                     .append(org.apache.axis2.databinding.utils.ConverterUtil.convertToString(
                                qnames[i]));
                    } else {
                        stringToWrite.append(org.apache.axis2.databinding.utils.ConverterUtil.convertToString(
                                qnames[i]));
                    }
                } else {
                    stringToWrite.append(org.apache.axis2.databinding.utils.ConverterUtil.convertToString(
                            qnames[i]));
                }
            }

            xmlWriter.writeCharacters(stringToWrite.toString());
        }
    }

    /**
     * Register a namespace prefix
     */
    private String registerPrefix(
        javax.xml.stream.XMLStreamWriter xmlWriter, String namespace)
        throws javax.xml.stream.XMLStreamException {
        String prefix = xmlWriter.getPrefix(namespace);

        if (prefix == null) {
            prefix = generatePrefix(namespace);

            javax.xml.namespace.NamespaceContext nsContext = xmlWriter.getNamespaceContext();

            while (true) {
                String uri = nsContext.getNamespaceURI(prefix);

                if ((uri == null) || (uri.length() == 0)) {
                    break;
                }

                prefix = org.apache.axis2.databinding.utils.BeanUtil.getUniquePrefix();
            }

            xmlWriter.writeNamespace(prefix, namespace);
            xmlWriter.setPrefix(prefix, namespace);
        }

        return prefix;
    }

    /**
     *  Factory class that keeps the parse method
     */
    public static class Factory {
        private static org.apache.commons.logging.Log log = org.apache.commons.logging.LogFactory.getLog(Factory.class);

        /**
         * static method to create the object
         * Precondition:  If this object is an element, the current or next start element starts this object and any intervening reader events are ignorable
         *                If this object is not an element, it is a complex type and the reader is at the event just after the outer start element
         * Postcondition: If this object is an element, the reader is positioned at its end element
         *                If this object is a complex type, the reader is positioned at the end element of its outer element
         */
        public static T001L parse(javax.xml.stream.XMLStreamReader reader)
            throws Exception {
            T001L object = new T001L();

            int event;
            javax.xml.namespace.QName currentQName = null;
            String nillableValue = null;
            String prefix = "";
            String namespaceuri = "";

            try {
                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                currentQName = reader.getName();

                if (reader.getAttributeValue(
                            "http://www.w3.org/2001/XMLSchema-instance", "type") != null) {
                    String fullTypeName = reader.getAttributeValue("http://www.w3.org/2001/XMLSchema-instance",
                            "type");

                    if (fullTypeName != null) {
                        String nsPrefix = null;

                        if (fullTypeName.indexOf(":") > -1) {
                            nsPrefix = fullTypeName.substring(0,
                                    fullTypeName.indexOf(":"));
                        }

                        nsPrefix = (nsPrefix == null) ? "" : nsPrefix;

                        String type = fullTypeName.substring(fullTypeName.indexOf(
                                    ":") + 1);

                        if (!"T001L".equals(type)) {
                            //find namespace for the prefix
                            String nsUri = reader.getNamespaceContext()
                                                           .getNamespaceURI(nsPrefix);

                            return (T001L) ExtensionMapper.getTypeObject(nsUri,
                                type, reader);
                        }
                    }
                }

                // Note all attributes that were handled. Used to differ normal attributes
                // from anyAttributes.
                java.util.Vector handledAttributes = new java.util.Vector();

                reader.next();

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "MANDT").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "MANDT").equals(
                            reader.getName())) {
                    object.setMANDT(MANDT_type13.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "WERKS").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "WERKS").equals(
                            reader.getName())) {
                    object.setWERKS(WERKS_type7.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "LGORT").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "LGORT").equals(
                            reader.getName())) {
                    object.setLGORT(LGORT_type3.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "LGOBE").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "LGOBE").equals(
                            reader.getName())) {
                    object.setLGOBE(LGOBE_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "SPART").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "SPART").equals(
                            reader.getName())) {
                    object.setSPART(SPART_type7.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "XLONG").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "XLONG").equals(
                            reader.getName())) {
                    object.setXLONG(XLONG_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "XBUFX").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "XBUFX").equals(
                            reader.getName())) {
                    object.setXBUFX(XBUFX_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "DISKZ").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "DISKZ").equals(
                            reader.getName())) {
                    object.setDISKZ(DISKZ_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "XBLGO").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "XBLGO").equals(
                            reader.getName())) {
                    object.setXBLGO(XBLGO_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "XRESS").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "XRESS").equals(
                            reader.getName())) {
                    object.setXRESS(XRESS_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "XHUPF").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "XHUPF").equals(
                            reader.getName())) {
                    object.setXHUPF(XHUPF_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "PARLG").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "PARLG").equals(
                            reader.getName())) {
                    object.setPARLG(PARLG_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "VKORG").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "VKORG").equals(
                            reader.getName())) {
                    object.setVKORG(VKORG_type9.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "VTWEG").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "VTWEG").equals(
                            reader.getName())) {
                    object.setVTWEG(VTWEG_type9.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "VSTEL").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "VSTEL").equals(
                            reader.getName())) {
                    object.setVSTEL(VSTEL_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "LIFNR").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "LIFNR").equals(
                            reader.getName())) {
                    object.setLIFNR(LIFNR_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "KUNNR").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "KUNNR").equals(
                            reader.getName())) {
                    object.setKUNNR(KUNNR_type7.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "MESBS").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "MESBS").equals(
                            reader.getName())) {
                    object.setMESBS(MESBS_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "MESST").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "MESST").equals(
                            reader.getName())) {
                    object.setMESST(MESST_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "OIH_LICNO").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "OIH_LICNO").equals(
                            reader.getName())) {
                    object.setOIH_LICNO(OIH_LICNO_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "OIG_ITRFL").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "OIG_ITRFL").equals(
                            reader.getName())) {
                    object.setOIG_ITRFL(OIG_ITRFL_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if ((reader.isStartElement() &&
                        new javax.xml.namespace.QName("", "OIB_TNKASSIGN").equals(
                            reader.getName())) ||
                        new javax.xml.namespace.QName("", "OIB_TNKASSIGN").equals(
                            reader.getName())) {
                    object.setOIB_TNKASSIGN(OIB_TNKASSIGN_type1.Factory.parse(
                            reader));

                    reader.next();
                } // End of if for expected property start element

                else {
                }

                while (!reader.isStartElement() && !reader.isEndElement())
                    reader.next();

                if (reader.isStartElement()) {
                    // 2 - A start element we are not expecting indicates a trailing invalid property
                    throw new org.apache.axis2.databinding.ADBException(
                        "Unexpected subelement " + reader.getName());
                }
            } catch (javax.xml.stream.XMLStreamException e) {
                throw new Exception(e);
            }

            return object;
        }
    } //end of factory class
}
